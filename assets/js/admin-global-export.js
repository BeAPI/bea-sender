	...
	jQuery('#wpbody').on( 'click', '.export-global', function (event) {
		...
		var exportCampaign = {
			...
			initMessageSection: function () {
				this.logDiv = jQuery('<div/>').addClass('metabox-holder has-right-sidebar').append(jQuery('<div/>').addClass('stuffbox').append(jQuery('<h3/>').append(jQuery('<label/>').html('Log file creation'))).append(jQuery('<div/>').addClass('inside logBlock')));
			...
			launchEvent : function () {
				...
				jQuery.ajax({
					...
					beforeSend: function () {
						_self.setMessage('message', 'Request to create the file.');
					},
					success   : function (checkFile) {
						...
						_self.setMessage('message', 'Checking if the lock file exists');
