...
<div class="wrap">
	...
	<div class="bea_sender_exports" >
		<h2><?php _e( 'Exports', 'bea_sender' ) ?></h2>
		<div class="bea-sender-export" id="bea-sender-global" >
			<h3> <?php _e(  'Global emails','bea_sender' ); ?></h3>
		...
		<div class="bea-sender-export" id="bea-sender-bounces" >
			<h3> <?php _e(  'Bounced emails','bea_sender' ); ?> </h3>

			<?php if( isset( $export_options['bounces'] ) && !empty( $export_options['bounces'] ) ): ?>
				<p class="description">
					<?php printf( __( 'The last file was generated on <strong> %s </strong>, you can download the CSV file on <strong><a href="%s" >this link</a></strong>','bea_sender' ), esc_attr( $export_options['bounces']['date'] ), esc_attr( $export_options['bounces']['url'] ) ); ?>
				</p>
			<?php endif; ?>
			<button data-type="bounces" data-nonce="<?php echo wp_create_nonce( 'bea-sender-export-bounces' ); ?>" class="generate export-bounces button button-primary">
				<?php _e( 'Export all bounces to CSV', 'bea_sender' ) ?>
			</button>
			<div class="log"></div>
		</div>
	</div>
	...
	<div class="bea_sender_purge" >
		<h2><?php _e( 'Purge', 'bea_sender' ) ?></h2>
		<div class="bea-sender-purge" id="bea-sender-purge" >
			<p class="description" ><?php _e( 'Here you can purge all bounced emails from the database between two dates' , 'bea_sender' ) ;?></p>
			<form method="post" action="<?php echo admin_url( 'admin-post.php' ); ?>">
				<label for="bea-sender-date-from-display" ><?php _e(  'From :','bea_sender' ); ?> </label>
				...
				<label for="bea-sender-date-to-display" > <?php _e(  'To :','bea_sender' ); ?> </label>
				...
				submit_button( __( 'Purge this range', 'bea_sender' ) ); ?>
			</form>
		</div>
	</div>
