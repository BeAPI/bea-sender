...
class BounceMailHandler {
    ...
    function processBounce($pos, $type, $totalFetched) {
        ...
        $bounce_header = imap_fetchbody ($this->_mailbox_link, $pos, 3);
        ...
        $xheader = 0;
        preg_match("/campaign-id:\s(\d+)/", $bounce_header, $match);
        if (!empty($match)) {
            $xheader = $match[1];
        }
    ...
    function mailbox_exist($mailbox, $create = true) {
        ...
        $port = $this->port . '/' . $this->service . '/' . $this->service_option;
        $mbox = imap_open('{' . $this->mailhost . ":" . $port . '}', $this->mailbox_username, $this->mailbox_password);
    ...
    function globalDelete() {
        ...
        $dateArr = explode('-', $todayDate); // date format is yyyy-mm-dd
        $delDate = mktime(0, 0, 0, $dateArr[1], $dateArr[2], $dateArr[0]);

        $port = $this->port . '/' . $this->service . '/' . $this->service_option;
        $mboxt = imap_open('{' . $this->mailhost . ":" . $port . '}', $this->mailbox_username, $this->mailbox_password);
