 ...
 * @return array    $result an array include the following fields: 'email',
 * 'bounce_type','remove','rule_no','rule_cat'
 *                      if we could NOT detect the type of bounce, return rule_no
 * = '0000'
 */
function bmhBodyRules( $body, $structure, $debug_mode = false ) {
	 ...
	 * Domain not existing
	 */
	if( preg_match( "/<(\S+@\S+\w)>.*Host or domain name not found/i", $body, $match ) ) {
	 ...
	 * rule: mailbox full;
	 * sample:
	 * The message to xxxxx@yourdomain.com is bounced because : Quota exceed the hard
	 * limit
	 */
	 ...
	 * sample:
	 * <xxxxx@yourdomain.com>:
	 * Unable to switch to /var/vpopmail/domains/domain.com: input/output error.
	 * (#4.3.0)
	 */
	 ...
	 */
	elseif( preg_match( "*Recipient address rejected*", $body, $match ) ) {
		$result['rule_cat'] = 'unknown';
		$result['rule_no'] = '0046';
		$result['email'] = $match[1];
		$result['bounce_type'] = '5.1.1';
	 ...
	 */
	elseif( preg_match( "*in reply to end of DATA command*", $body, $match ) ) {
		$result['rule_cat'] = 'unknown';
		$result['rule_no'] = '0047';
		$result['email'] = $match[1];
	}
	 ...
	 */
	elseif( preg_match( "*in reply to RCPT TO command*", $body, $match ) ) {
		$result['rule_cat'] = 'unknown';
		$result['rule_no'] = '0048';
		$result['email'] = $match[1];
	}
	...
	if( empty( $result['email'] ) ) {
		...
		switch ($action) {
			case 'failed' :
				 ...
				 * sample 2:
				 * Diagnostic-Code: X-Postfix; host mta5.us4.domain.com.int[111.111.111.111]
				 * said:
				 ...
				 * sample 2:
				 * Diagnostic-Code: X-Postfix; cannot access mailbox /var/spool/mail/b8843022
				 * for^M
				...
				/* rule: unknown
				 * sample:
				 * Diagnostic-Code: X-Notes; User xxxxx (xxxxx@yourdomain.com) not listed in
				 ...
				 * sample 3:
				 * Diagnostic-Code: SMTP; 550 relaying to <xxxxx@yourdomain.com> prohibited by
				 * administrator
				 */
				 ...
				 * xxxxx@yourdomain.com cannot be delivered. This account has been disabled or
				 * discontinued [#102]. - mta173.mail.tpe.domain.com
				 */
				 ...
				 * domain.com account (www.xxxxx@yourdomain.com) [0] - mta134.mail.tpe.domain.com
				 */
				 ...
				 * sample 2:
				 * Diagnostic-Code: SMTP; 550 5.7.1 Requested action not taken: mailbox not
				 * available
				 */
				 ...
				 * sample 2:
				 * Diagnostic-Code: SMTP; 553 5.3.0 <xxxxx@yourdomain.com>... Addressee unknown,
				 * relay=[111.111.111.000]
				 */
				 ...
				 * sample 2:
				 * Diagnostic-Code: SMTP; 550 Mailbox ( xxxxx@yourdomain.com ) not found or
				 * inactivated
				 */
				...
				/* rule: unknown
				 * sample:
				 * Diagnostic-Code: smtp; 5.1.0 - Unknown address error 540-'Error: Wrong
				 * recipients' (delivery attempts: 0)
				 */
				...
				/* rule: unknown
				 * sample:
				 * Diagnostic-Code: SMTP; 550 5.1.1 <xxxxx@yourdomain.com>... email address
				 * lookup in domain map failed^M
				 */
				...
				/* rule: unknown
				 * sample:
				 * Diagnostic-Code: SMTP; 550 User not a member of domain:
				 * <xxxxx@yourdomain.com>^M
				 */
				...
				/* rule: unknown
				 * sample:
				 * Diagnostic-Code: SMTP; 550-"The recipient cannot be verified.  Please check
				 * all recipients of this^M
				 */
				...
				/* rule: unknown
				 * sample:
				 * Diagnostic-Code: SMTP; 550-I'm sorry but xxxxx@yourdomain.com does not have an
				 * account here. I will not
				 */
				...
				/* rule: inactive
				 * sample:
				 * Diagnostic-Code: SMTP; 550 <xxxxx@yourdomain.com>... User account not
				 * activated
				 */
				...
				/* rule: inactive
				 * sample:
				 * Diagnostic-Code: SMTP; 553 5.3.0 <xxxxx@yourdomain.com>... Recipient address
				 * no longer exists
				 */
				...
				/* rule: inactive
				 * sample:
				 * Diagnostic-Code: SMTP; 553 VS10-RT Possible forgery or deactivated due to
				 * abuse (#5.1.1) 111.111.111.211^M
				 */
				...
				/* rule: user_reject
				 * sample:
				 * Diagnostic-Code: SMTP; 501 xxxxx@yourdomain.com Sender email is not in my
				 * domain
				 */
				...
				/* rule: command_reject
				 * sample:
				 * Diagnostic-Code: SMTP; 550-ds176.domain.com [111.111.111.211] is currently not
				 * permitted to
				...
				/* rule: content_reject
				 * sample:
				 * Diagnostic-Code: smtp; 554 5.6.0 Message with invalid header rejected,
				 * id=13462-01 - MIME error: error: UnexpectedBound: part didn't end with
				 * expected boundary [in multipart message]; EOSToken: EOF; EOSType: EOF
				 */
				...
				/* rule: defer
				 * sample:
				 * Diagnostic-Code: SMTP; 451 mta172.mail.tpe.domain.com Resources temporarily
				 * unavailable. Please try again later.  [#4.16.4:70].
				 */
				 ...
				 * sample:
				 * Diagnostic-Code: SMTP; 554 sender is rejected:
				 * 0,mx20,wKjR5bDrnoM2yNtEZVAkBg==.32467S2
				 */
				...
				/* rule: antispam, deny ip
				 * sample:
				 * Diagnostic-Code: SMTP; 554 <unknown[111.111.111.000]>: Client host rejected:
				 * Access denied
				 */
				 ...
				 * sample:
				 * Diagnostic-Code: SMTP; 554 Connection refused(mx). MAIL FROM
				 * [xxxxx@yourdomain.com] mismatches client IP [111.111.111.000].
				 */
				...
				/* rule: antispam, deny ip
				 * sample:
				 * Diagnostic-Code: SMTP; 554 Please visit http://
				 * antispam.domain.com/denyip.php?IP=111.111.111.000 (#5.7.1)
				 */
				...
				/* rule: antispam, deny ip
				 * sample:
				 * Diagnostic-Code: SMTP; 554 Service unavailable; Client host [111.111.111.211]
				 * blocked using dynablock.domain.com; Your message could not be delivered due to
				 * complaints we received regarding the IP address you're using or your ISP. See
				 * http:// blackholes.domain.com/ Error: WS-02^M
				 */
				 ...
				 * sample:
				 * Diagnostic-Code: SMTP; 550 Requested action not taken: mail
				 * IsCNAPF76kMDARUY.56621S2 is rejected,mx3,BM
				 */
				...
				/* rule: antispam
				 * sample:
				 * Diagnostic-Code: SMTP; 554 5.7.1 Rejected as Spam see: http://
				 * rejected.domain.com/help/spam/rejected.html
				 */
				...
				/* rule: antispam
				 * sample:
				 * Diagnostic-Code: SMTP; 553 5.7.1 <xxxxx@yourdomain.com>... SpamTrap=reject
				 * mode, dsn=5.7.1, Message blocked by BOX Solutions (www.domain.com) SpamTrap
				 * Technology, please contact the domain.com site manager for help:
				 * (ctlusr8012).^M
				 */
				...
				/* rule: antispam, mailfrom mismatch
				 * sample:
				 * Diagnostic-Code: SMTP; 550 Error: MAIL FROM is mismatched with message header
				 * from address!
				 */
				...
				/* rule: antispam
				 * sample:
				 * Diagnostic-Code: SMTP; 554 5.7.1 Message scored too high on spam scale.  For
				 * help, please quote incident ID 22492290.
				 */
				...
				/* rule: antispam
				 * sample:
				 * Diagnostic-Code: SMTP; 554 5.7.1 reject: Client host bypassing service
				 * provider's mail relay: ds176.domain.com
				 8? else

				 if (preg_match ("/Client host bypass/is",$diag_code)) {
				 $result['rule_cat']    = 'antispam';
				 $result['rule_no']     = '0229';
				 }
				 /* rule: antispam
				...
				/* rule: antispam
				 * sample:
				 * Diagnostic-Code: SMTP; 553-Message filtered. Please see the FAQs section on
				 * spam
				 */
				 ...
				 * sample:
				 * Diagnostic-Code: SMTP; 554 5.7.1 The message from (<xxxxx@yourdomain.com>)
				 * with the subject of ( *(ca2639) 7|-{%2E* : {2"(%EJ;y} (SBI$#$@<K*:7s1!=l~)
				 * matches a profile the Internet community may consider spam. Please revise your
				...
				/* rule: delayed
				 * sample:
				 * Diagnostic-Code: X-Postfix; delivery temporarily suspended: conversation
				 * with^M
				 * 111.111.111.11[111.111.111.11] timed out while sending end of data -- message
				 * may be^M
				 * sent more than once
				 */
				 ...
				 * sample 2:
				 * Error in fabiomod_sql_glob_init: no data source specified - database access
				 * disabled
				 ...
				 * sample:
				 * ----- Transcript of session follows -----
				 * xxxxx@yourdomain.com... Deferred: Name server: domain.com.: host name lookup
				 * failure
				 */
