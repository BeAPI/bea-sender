...
if ( ! class_exists( 'WeDevs_Settings_API' ) ) :
	class WeDevs_Settings_API {
		...
		public function callback_file( $args ) {
			...
			wp_enqueue_media();
			add_action( 'admin_footer', [ __CLASS__, 'script_medias' ] );
		...
		public function show_navigation() {
			...
			foreach ( $this->settings_sections as $section ) {
				...
				$html .= sprintf( '<a href="%1$s" class="nav-tab %2$s" id="%3$s-tab">%4$s</a>' . PHP_EOL, esc_url( add_query_arg( [ 'tab' => $section['id'] ] ) ), $class, $section['id'], $title );
			}
			$html .= '</h2>' . PHP_EOL;
