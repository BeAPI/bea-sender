...
class Bea_Sender_Campaign {
	...
	private function send() {
		...
		foreach( $this->emailContents as $send ) {
			...
			$mailed = Bea_Sender_Email::wpMail( $send->email, $this->subject, array( 'html' => $send->html, 'raw' => $send->text ) );
	...
	private function createCampaign( $data = array() ) {
		...
		// Insert the row
		$inserted = $wpdb->insert( $wpdb->bea_s_campaigns , array( 'add_date' => $add_date ,'scheduled_from' => $data['scheduled_from'] ,'current_status' => 'registered', 'from_name' => $data['from_name'], 'from' => $data['from'], 'subject' => $data['subject'] ), array( '%s', '%s', '%s', '%s' ) );
	...
	private function linkReceiver( $c_id, $r_id ) {
		...
		$inserted = $wpdb->insert( $wpdb->bea_s_re_ca, array( 'id_campaign' => $this->id, 'id_receiver' => $r_id, 'id_content' => $c_id, 'current_status' => 'pending', 'response' => '' ) );
	...
	private function removeSendFilters() {
		// Remove filters
		remove_filter( 'wp_mail_content_type' , array( &$this, 'returnHTML' ) );
		remove_filter( 'wp_mail_from', 			array( &$this, 'mailFrom' ) );
		remove_filter( 'wp_mail_from_name', 	array( &$this, 'mailFromName' ) );
	}
